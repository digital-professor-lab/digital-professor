...
\section{Implementation scaffold}
The first implementation should run locally in a browser and use mock data before connecting to a model or document-processing backend. This makes the interaction design demonstrable while the parallel research on syllabus extraction, model selection, costs, and existing products continues. The mock responses and service contracts can later be replaced without changing the tabs or student workflow.
...
research/interaction-methods/
  ...
  src/
    digital_professor/
      ...
      skills.json
notebooks/
  recognition_demo.ipynb
resources/
  interaction_test/
\end{verbatim}

The existing files in \texttt{resources/interaction\_test/} provide the first coherent test course: a Nonlinear Optimization I syllabus, lecture notes, homework, correct handwritten work, incorrect work, and ambiguous handwriting. The repository should later add audio and image-format fixtures, but these PDFs are sufficient to scaffold upload states and the main source-grounded chat path.

\subsection{Frontend architecture}
\begin{itemize}
\item \textbf{Stack:} React with TypeScript and Vite for a lightweight single-page application served on localhost. React supplies reusable stateful components, while Vite supplies the local development server and production build.\cite{react,vite}
\item \textbf{Navigation:} a top-level tab state switches between Sources, Provider Settings, and Chat without discarding the active course or conversation. Chat is the default tab after onboarding.
\item \textbf{Sources tab:} if no course exists, show \emph{Upload your syllabus to create a course}. While using mocks, uploading \texttt{samp\_syllabus.pdf} loads a fixture containing the proposed name \emph{Nonlinear Optimization I}, course code \texttt{EN.553.761}, term, and topics. The student must confirm or edit the name. After confirmation, the tab accepts additional files and shows \emph{queued}, \emph{processing}, \emph{ready}, or \emph{failed} states.
\item \textbf{Provider Settings tab:} begin with \emph{Mock provider} and placeholders for the tutor, transcription, OCR, and speech providers. Store only provider IDs and user preferences in browser state. API keys remain backend environment variables.
\item \textbf{Chat tab:} render user and tutor messages, citations, math blocks, attachments, and comprehension checks. The composer accepts text and file references. Microphone and equation controls may initially display \emph{Coming Soon} or return mock results.
\item \textbf{Office Hours:} keep a visible disabled control labeled \emph{Coming Soon}. It should not require any real-time implementation in the scaffold.
\item \textbf{State:} for the first demonstration, keep the active course, source list, settings, and messages in a single application state object. Add browser persistence only after the basic reset and upload flows work predictably.
\end{itemize}

\subsection{Frontend components and states}
The UI should be built around states that can be demonstrated without a backend:
\begin{itemize}
\item \textbf{Empty state:} no course exists; the Sources tab requests a syllabus.
\item \textbf{Extracting state:} show the uploaded filename and a processing indicator.
\item \textbf{Confirmation state:} display extracted fields with editable course name and \emph{Confirm course} action.
\item \textbf{Ready state:} display the course name in the application header and allow notes, homework, and student work to be added.
\item \textbf{Chatting state:} accept an open-ended question, show which sources are active, and return a fixture response with a citation and comprehension check.
\item \textbf{Recovery state:} if recognition confidence is low, show the interpreted text or equation and require correction before continuing.
\end{itemize}

One scripted mock response should use the uploaded optimization materials. For example, after the student attaches \texttt{samp\_solution.pdf} and asks whether the log-sum-exp proof is complete, the tutor can ask the student to justify why $\operatorname{diag}(p)-pp^\top$ is positive semidefinite, cite the relevant lecture-note page, and finish with a comprehension question.

\subsection{Backend architecture for the next stage}
After the frontend scaffold is stable, add a small Python backend using FastAPI. FastAPI provides typed request models and automatically generated interactive API documentation, which is useful while the contracts are still changing.\cite{fastapi} The backend remains a thin boundary around reusable Python functions:
\begin{itemize}
\item \texttt{POST /courses/extract}: accept a syllabus and return proposed course metadata.
\item \texttt{POST /courses}: confirm the edited metadata and create an in-memory course record.
\item \texttt{POST /sources}: register a document or student artifact and return its processing state.
\item \texttt{POST /chat}: accept the normalized tutoring request and return a structured tutor response.
\item \texttt{GET /providers}: return the provider registry supplied by the model-and-cost research.
\item \texttt{GET /health}: verify that the local backend is available.
\end{itemize}

For the first backend demonstration, use in-memory dictionaries and fixture responses. File persistence, a vector database, authentication, background workers, and production deployment are outside this scaffold. When a real model is added, only the backend reads credentials and calls provider APIs. The browser sends provider IDs, never secret keys.

\subsection{Shared data contracts}
The frontend, backend, and notebook should use the same conceptual objects:
\begin{itemize}
\item \textbf{CourseMetadata:} course ID, confirmed name, extracted name, code, term, instructor, topics, confidence, and fields needing confirmation.
\item \textbf{SourceArtifact:} artifact ID, course ID, filename, media type, role (authoritative source or student work), status, page count, and optional selected region.
\item \textbf{TutorRequest:} session ID, course ID, message, inferred intent, active source IDs, attachment IDs, recognized text/math, and response preferences.
\item \textbf{TutorResponse:} response type, message, rendered-math strings, citations, recognition confirmation, comprehension check, and confidence.
\item \textbf{ProviderDefinition:} provider ID, model ID, modalities, streaming support, enabled state, and cost fields supplied by the parallel research.
\end{itemize}

Use JSON fixtures conforming to these contracts before introducing network calls. This gives the syllabus, provider, frontend, and backend work a shared integration target.

\subsection{Local setup and development sequence}
The intended local workflow is:
\begin{enumerate}
\item Create the \texttt{research/interaction-methods/webapp/frontend} application with the React--TypeScript Vite template, install dependencies, and run the Vite development server. Vite normally serves the application at \url{http://localhost:5173}.\cite{vite}
\item Implement the tab layout and application state with no backend. Use \texttt{mockApi.ts} to simulate short processing delays and load JSON fixtures.
\item Implement syllabus upload, extraction review, course confirmation, additional source cards, and the scripted chat exchange.
\item Verify the complete interaction on a laptop browser and an iPad-sized responsive viewport.
\item Add \texttt{research/interaction-methods/webapp/backend} with FastAPI and keep its reusable Python machinery in \texttt{research/interaction-methods/src}. Run it locally on a separate port, commonly \url{http://localhost:8000}, and configure the frontend API base URL through a development environment variable.
\item Replace one mock at a time: health check, provider registry, syllabus fixture response, source registration, then chat. Preserve a mock mode so the UI remains demonstrable without API credentials.
\item Add the Jupyter notebook only after the Python schemas and service boundaries exist. The notebook should import or call those functions rather than contain a second implementation.
\end{enumerate}

The repository should include an \texttt{.env.example} listing variable names without values. Actual provider keys stay in an ignored local \texttt{.env} file. Setup instructions should specify supported Node.js and Python versions after they are chosen and tested, rather than relying on whichever versions happen to be installed by a contributor.

\subsection{Jupyter notebook demonstration}
The notebook is the first executable backend experiment, not the primary user interface. It should demonstrate the processing chain with visible intermediate objects:
\begin{enumerate}
\item Load \texttt{samp\_syllabus.pdf} and produce or load the \texttt{CourseMetadata} fixture.
\item Confirm the course name and register \texttt{samp\_notes.pdf}, \texttt{samp\_homework.pdf}, and one handwritten solution as \texttt{SourceArtifact} objects.
\item Construct a \texttt{TutorRequest} asking for feedback on the student's proof.
\item Run the mock orchestrator first, then replace it with one selected model/provider.
\item Display retrieved evidence, interpreted equations, inferred intent, the final \texttt{TutorResponse}, citations, and the comprehension check.
\item Log latency and estimated or reported usage so the model-and-cost research can be compared with an actual interaction.
\end{enumerate}

The notebook and browser should produce the same response schema. Once that contract works, the FastAPI endpoint can wrap the notebook-tested function and the frontend can replace its mock client with the HTTP client.
...
In the future public-facing version, clicking emph{Office Hours} should enter a dedicated live session and activate a digital professor avatar. The avatar should synchronize spoken responses with facial movement and simple instructional gestures while the student communicates through real-time text, audio, and optional video. A shared work surface can display equations, code, uploaded pages, and annotations beside the avatar. The system should retain the current course sources and conversation context, provide a live transcript, support interruption and turn-taking, and continue checking whether the student follows. Camera use should be optional and permission-based; students should still be able to complete an Office Hours session using text, voice, and shared artifacts alone. This concept requires later evaluation of streaming latency, speech and avatar synchronization, accessibility, privacy, session recording, and recovery from dropped connections.
...
\bibitem{react} Meta Open Source. \emph{React: Learn}. \url{https://react.dev/learn}.
\bibitem{vite} Vite contributors. \emph{Vite: Getting Started}. \url{https://vite.dev/guide/}.
\bibitem{fastapi} FastAPI contributors. \emph{FastAPI Tutorial}. \url{https://fastapi.tiangolo.com/tutorial/}.
\end{thebibliography}
\end{document}
